...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, end2end=True):
        ...
        if block == BasicBlock:
            ...
            self.alpha = nn.Sequential(nn.Linear(2048, 1),
                                        nn.Sigmoid())
            self.beta = nn.Sequential(nn.Linear(4096, 1),
                                      nn.Sigmoid())
    ...
    def forward(self, x):
        # pdb.set_trace()
        # x.shape torch.Size([20, 3, 224, 224, 6])
        vs = []
        alphas = []
        ...
        # output.shape torch.Size([22, 1024])
        # pred_score.shape torch.Size([22, 8])
        # alphas_part_max.shape torch.Size([22, 1])
        # alphas_org.shape torch.Size([22, 1])
